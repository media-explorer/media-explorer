...
static void
add_pictures (ClutterActor *column)
{
  GList *files = get_pictures ();

  while (files && tiles < 10)
    {
      gchar *basename, *url;
      ClutterActor *tile;
      MexContent *content;
      ...
      clutter_container_add_actor (CLUTTER_CONTAINER (column), tile);
...
main (int argc, char **argv)
{
  ...
  app = mx_application_new (&argc, &argv, "test-mex-column", 0);
